...
import (
	dpfm_api_caller "data-platform-api-bill-of-material-reads-rmq-kube/DPFM_API_Caller"
	...
	"data-platform-api-bill-of-material-reads-rmq-kube/config"
	"encoding/json"
	"fmt"
	"time"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, errs := caller.AsyncBillOfMaterialReads(accepter, &input, &output, l)
	if len(errs) != 0 {
